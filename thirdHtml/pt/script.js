...
// Observar cards de produtos e seções
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Efeito ripple nos cards de produtos
...
// ===== Navegação: Login e Seletor de Idioma =====
(function() {
  ...
  // Verificar login armazenado
  const storedUser = getStoredUser()
  if (storedUser) {
    showLoggedInState(storedUser)
  }

  // Botão de login abre a plataforma em nova aba
  // A plataforma redireciona de volta com as informações do usuário via postMessage ou parâmetros de URL
  if (navLogin) {
    navLogin.addEventListener('click', function(e) {
      // Simular login para fins de demonstração
      // Em produção, a página de login da plataforma gerencia isso
  ...
  function updateLangUI(lang) {
    ...
    const info = LANG_MAP[lang] || LANG_MAP.pt
  ...
  // Inicializar UI de idioma do armazenamento
  ...
  if (langDropdown) {
    langDropdown.addEventListener('click', function(e) {
      ...
      if (lang) {
        ...
        // Navegar para o site do idioma selecionado, preservando a página atual
  ...
  // --- Escutar mensagens de login da plataforma ---
  ...
  // --- Verificar parâmetros de callback de login na URL ---
  try {
