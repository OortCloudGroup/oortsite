...
function animateCounters() {
  ...
  counters.forEach((counter) => {
    ...
    const hasM = text.includes('M') || text.includes('مليون')
    // Extract numeric value (supports Eastern Arabic numerals)
    ...
    const timer = setInterval(() => {
      ...
      if (current >= numericValue) {
        ...
        clearInterval(timer)
        // Restore original text on completion
      ...
      let arabicDisplay = String(display).replace(/[0-9]/g, d => '٠١٢٣٤٥٦٧٨٩'[d])

      let suffix = ''
      if (hasPlus) suffix = '+'
      if (hasPercent) suffix = '٪'

      counter.textContent = arabicDisplay + suffix
    }, 16)
  })
}
...
(function() {
  ...
  if (navLogin) {
    navLogin.addEventListener('click', function(e) {
      ...
      const fakeUser = { name: 'مدير', username: 'admin' }
  ...
  // Arabic is the current language for this deployment
  ...
  function updateLangUI(lang) {
    ...
    const info = LANG_MAP[lang] || LANG_MAP.ar
  ...
  // Init lang UI — Arabic is current
  updateLangUI('ar')
    ...
    langDropdown.addEventListener('click', function(e) {
      ...
      if (lang) {
        ...
        if (info && info.url) {
          ...
          // If Arabic selected, stay on current site
          if (lang === 'ar') {
